          ...
          {[
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-[10px] bg-white border border-slate-200 text-[#12163A] px-2.5 py-1 rounded-lg font-bold shadow-2xs whitespace-nowrap">
                  {c.match}
                </span>
                <span className="text-[10px] font-bold text-[#3665EE]">{c.growth}</span>
              </div>
              <h3 className="font-bold text-sm truncate text-[#12163A]">{c.title}</h3>
              <div className="font-bold text-xs text-[#3665EE]">{c.salary}</div>
              <div className="flex flex-wrap gap-1">
                {c.skills.map((sk, idx) => (
                  <span key={idx} className="text-[10px] px-2 py-0.5 rounded-md bg-white/80 text-[#12163A] font-semibold">
                    {sk}
                  </span>
                ))}
              </div>
      ...
      <div className="rounded-[24px] bg-white border border-slate-200 p-6 space-y-6 text-xs font-sans shadow-xs">
        ...
        <div className="p-6 rounded-[24px] bg-[#DEE9FF] border border-[#C6D9FF] text-[#12163A] space-y-4">
          <div className="flex items-center justify-between text-[11px]">
            <span className="font-bold text-[#3665EE]">Category: Logical & Spatial Reasoning</span>
            <span className="text-[#4B5563]">Score Weight: 25 Points</span>
          </div>

          <p className="font-bold text-sm leading-relaxed text-[#12163A]">
          ...
          <div className="space-y-2.5 pt-2">
            {[
              ...
              <button 
                ...
                className={`w-full text-left p-3.5 rounded-xl border text-xs font-semibold transition cursor-pointer ${
                  idx === 1 
                    ? 'bg-[#12163A] border-[#12163A] text-white font-bold' 
          ...
          <div className="pt-4 flex items-center justify-between border-t border-[#C6D9FF]">
            <button onClick={() => onShowToast("Navigated to previous question")} className="px-4 py-2 rounded-xl font-bold border border-slate-300 bg-white text-[#12163A] hover:bg-slate-50 transition cursor-pointer">
              Previous Question
            </button>
            <button onClick={() => onShowToast("Assessment answers submitted! AI Score generated.")} className="px-5 py-2.5 rounded-xl font-bold bg-[#12163A] text-white shadow-md cursor-pointer transition hover:bg-[#1A2050]">
              Submit Assessment
            </button>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={i} className={`p-5 rounded-[24px] border space-y-3 ${item.bg} ${item.border} text-[#12163A]`}>
              <div className="flex items-center justify-between">
                <p className="font-bold text-xs text-[#12163A]">{item.q}</p>
                <span className="text-[10px] bg-white border border-slate-200 text-[#12163A] px-2.5 py-1 rounded-lg font-bold shadow-2xs whitespace-nowrap">{item.code}</span>
              </div>
                ...
                <div className="flex items-center gap-3">
                  {[1, 2, 3, 4, 5].map((val) => (
                    <button
                      ...
                      className={`w-7 h-7 rounded-full font-bold transition cursor-pointer ${
                        val === 4 && i === 0
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={i} className={`p-5 rounded-[24px] border flex items-center justify-between cursor-pointer transition-all duration-200 hover:-translate-y-0.5 ${rec.bg} ${rec.border} text-[#12163A] hover-card-lift`} onClick={() => onShowToast(`Viewing detailed AI breakdown for ${rec.role}`)}>
              <div>
                <h4 className="font-bold text-sm text-[#12163A]">{rec.role}</h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={i} className={`p-5 rounded-[24px] border space-y-2 cursor-pointer transition-all duration-200 ${col.bg} ${col.border} text-[#12163A] min-w-0 hover-card-lift`} onClick={() => onShowToast(`Added ${col.name} to Target Wishlist`)}>
              <span className="text-[10px] bg-[#12163A] text-white px-2.5 py-0.5 rounded-full font-bold">{col.rank}</span>
              <h3 className="font-bold text-sm truncate text-[#12163A]">{col.name}</h3>
              <div className="font-bold text-[#3665EE]">Avg CTC: {col.avgCtc}</div>
              <div className="text-[11px] text-[#4B5563]">Cutoff: {col.cutoff}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={i} className={`p-5 rounded-[24px] border flex items-center justify-between ${rd.bg} ${rd.border} text-[#12163A] hover-card-lift`}>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-[#12163A] text-white flex items-center justify-center font-bold text-xs shrink-0">
                  {i + 1}
                </div>
                <div>
                  <h4 className="font-bold text-sm text-[#12163A]">{rd.step}</h4>
                  <p className="text-[11px] text-[#4B5563]">{rd.desc}</p>
                </div>
              </div>
              <span className="text-[10px] px-3 py-1 rounded-full font-bold border bg-white border-slate-200 text-[#12163A] shrink-0">
                {rd.status}
              </span>
            </div>
          ))}
    ...
    <div className="rounded-[24px] bg-white border border-slate-200 p-6 space-y-6 text-xs font-sans shadow-xs">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-6 rounded-[24px] bg-[#12163A] text-white space-y-3 cursor-pointer hover-card-lift" onClick={() => onShowToast("Resumed Python AI module")}>
          <span className="text-[10px] uppercase font-bold text-[#DEE9FF]">Continue Learning</span>
          <h3 className="font-bold text-base text-white">Python for Data Science & AI</h3>
          <div className="w-full h-2.5 bg-white/20 rounded-full overflow-hidden">
            <div className="h-full bg-[#3665EE] rounded-full" style={{ width: '85%' }} />
          </div>
          <span className="text-[10px] text-[#DEE9FF] block font-bold">85% Completed</span>
        </div>
        ...
        <div className="p-6 rounded-[24px] bg-[#DEE9FF] border border-[#C6D9FF] text-[#12163A] space-y-3 cursor-pointer hover-card-lift" onClick={() => onShowToast("Opening System Design course")}>
          <span className="text-[10px] uppercase font-bold text-[#3665EE]">Recommended Course</span>
          <h3 className="font-bold text-base text-[#12163A]">System Design & Microservices</h3>
          ...
          <span className="text-[10px] text-[#4B5563] block font-bold">60% Completed</span>
        </div>
        ...
        <div className="p-6 rounded-[24px] bg-[#E4F4EC] border border-[#C3E6D5] text-[#12163A] space-y-3 cursor-pointer hover-card-lift" onClick={() => onShowToast("Viewing Cloud Cert badge")}>
          <span className="text-[10px] uppercase font-bold text-[#12163A]">Achievement Badge</span>
          <h3 className="font-bold text-base text-[#12163A]">Cloud Architecture (AWS)</h3>
          <div className="w-full h-2.5 bg-white/80 rounded-full overflow-hidden">
            <div className="h-full bg-[#3665EE] rounded-full" style={{ width: '45%' }} />
          </div>
          <span className="text-[10px] text-[#12163A] block font-bold">Foundational Certification Earned</span>
        </div>
      </div>
    </div>
  );
