            ...
            <tbody className={`divide-y ${borderDivider}`}>
              {logs.map((log) => (
                <tr key={log.id} className={`transition ${isDarkMode ? 'hover:bg-slate-800/60' : 'hover:bg-blue-50/40'}`}>
                  ...
                  <td className={`py-3.5 px-4 font-semibold ${textHeading}`}>{log.admin}</td>
                  <td className="py-3.5 px-4 font-medium text-blue-500">{log.action}</td>
                  <td className={`py-3.5 px-4 font-normal ${isDarkMode ? 'text-slate-300' : 'text-slate-800'}`}>{log.target}</td>
                  <td className="py-3.5 px-4">
                    <span className="bg-blue-500/20 text-blue-300 font-medium px-2 py-0.5 rounded-md text-xs">
                      {log.role}
                    </span>
                  </td>
                  <td className={`py-3.5 px-4 font-mono text-xs ${textMuted}`}>{log.ip}</td>
                  <td className="py-3.5 px-4">
      ...
      <div className="space-y-3">
        {logs.slice(0, 5).map((log) => (
          <div key={log.id} className={`p-3 rounded-xl border flex items-center justify-between transition ${
            ...
            <div>
              <div className={`text-sm font-semibold ${textHeading}`}>{log.action}</div>
              <div className={`text-xs font-normal ${textMuted}`}>Actor: {log.admin} • Target: {log.target}</div>
            </div>
            <div className="text-right">
              <span className="bg-emerald-500/20 text-emerald-400 text-xs font-medium px-2 py-0.5 rounded-full block mb-1">
                {log.status}
              </span>
