...
import { RegisterCompanyRequest } from '../../types/auth.types';
...
export const CompanySignup: React.FC<CompanySignupProps> = ({
  formData,
  onChange,
  onBack,
  onSuccess
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const returnedRole = res?.data?.role || 'COMPANY_ADMIN';
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700 text-[11px] font-semibold mb-1 border border-blue-200/60">
              Step 2 of 3 • Company Admin Registration
            </div>
            <h2 className="text-2xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
              <FiGrid className="text-blue-600 w-6 h-6" /> Enterprise Account Setup
            </h2>
            <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
              Configure corporate credentials and register your company workspace.
            </p>
          </div>
        ...
        <div className="space-y-4 max-h-[52vh] overflow-y-auto pr-1">
          ...
          <div className="bg-slate-50/70 p-3.5 rounded-2xl border border-slate-200/80 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 bg-white border rounded-xl text-slate-900 text-xs focus:outline-none focus:ring-2 transition h-10 ${
                    fieldErrors.industry ? 'border-rose-300' : 'border-slate-200 focus:ring-blue-500/20'
                  }`}
                />
                {fieldErrors.industry && <p className="text-[10px] text-rose-600 mt-1">{fieldErrors.industry}</p>}
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">
                  GST / Corporate Registration ID <span className="text-rose-500">*</span>
                </label>
                <input
                  type="text"
                  value={formData.gst}
                  ...
                  className={`w-full px-3 py-2 bg-white border rounded-xl text-slate-900 text-xs font-mono uppercase focus:outline-none focus:ring-2 transition h-10 ${
                    fieldErrors.gst ? 'border-rose-300' : 'border-slate-200 focus:ring-blue-500/20'
                  }`}
                />
                {fieldErrors.gst && <p className="text-[10px] text-rose-600 mt-1">{fieldErrors.gst}</p>}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <FiGlobe className="w-3.5 h-3.5 text-blue-500 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
                <input
                  type="url"
                  ...
                  className={`w-full pl-9 pr-3 py-2 bg-white border rounded-xl text-slate-900 text-xs focus:outline-none focus:ring-2 transition h-10 ${
                    fieldErrors.website ? 'border-rose-300' : 'border-slate-200 focus:ring-blue-500/20'
                  }`}
                />
              </div>
              {fieldErrors.website && <p className="text-[10px] text-rose-600 mt-1">{fieldErrors.website}</p>}
            </div>

            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">
                Company Profile &amp; Overview <span className="text-rose-500">*</span>
              </label>
              <textarea
                rows={2}
                ...
                className={`w-full px-3 py-2 bg-white border rounded-xl text-slate-900 text-xs focus:outline-none focus:ring-2 transition resize-none ${
                  fieldErrors.companyProfile ? 'border-rose-300' : 'border-slate-200 focus:ring-blue-500/20'
                }`}
              />
              {fieldErrors.companyProfile && <p className="text-[10px] text-rose-600 mt-1">{fieldErrors.companyProfile}</p>}
            </div>
          </div>
        </div>
      </div>
