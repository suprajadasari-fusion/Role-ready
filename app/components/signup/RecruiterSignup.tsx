...
import { RegisterRecruiterRequest } from '../../types/auth.types';
...
interface RecruiterSignupProps {
  ...
  onChange: (data: Partial<RecruiterFormData>) => void;
  onBack: () => void;
  onSuccess: (data: { email: string; role: string }) => void;
}

export const RecruiterSignup: React.FC<RecruiterSignupProps> = ({
  formData,
  onChange,
  onBack,
  onSuccess
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const returnedRole = res?.data?.role || 'RECRUITER';
    ...
    <form onSubmit={handleSubmit} className="flex flex-col h-full justify-between">
      <div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700 text-[11px] font-semibold mb-1 border border-blue-200/60">
              Step 2 of 3 • Recruiter Registration
            </div>
            <h2 className="text-2xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
              <FiBriefcase className="text-blue-600 w-6 h-6" /> Recruiter Desk Setup
            </h2>
            <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
              Set up your hiring credentials to source and evaluate verified candidates.
            </p>
          </div>
        ...
        <div className="space-y-4 max-h-[52vh] overflow-y-auto pr-1">
          ...
          {/* Section 2: Company & Recruitment Information */}
          <div className="bg-slate-50/70 p-3.5 rounded-2xl border border-slate-200/80 space-y-3">
            <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider">
              2. Company &amp; Recruitment Details
            </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="sm:col-span-2">
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 bg-white border rounded-xl text-slate-900 text-xs focus:outline-none focus:ring-2 transition h-10 ${
                    fieldErrors.companyName ? 'border-rose-300' : 'border-slate-200 focus:ring-blue-500/20'
                  }`}
                />
                {fieldErrors.companyName && <p className="text-[10px] text-rose-600 mt-1">{fieldErrors.companyName}</p>}
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">
                  Years Experience <span className="text-rose-500">*</span>
                </label>
                <input
                  type="number"
                  min={1}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">
                Your Professional Designation <span className="text-rose-500">*</span>
              </label>
              <input
                type="text"
                ...
                className={`w-full px-3 py-2 bg-white border rounded-xl text-slate-900 text-xs focus:outline-none focus:ring-2 transition h-10 ${
                  fieldErrors.designation ? 'border-rose-300' : 'border-slate-200 focus:ring-blue-500/20'
                }`}
              />
              {fieldErrors.designation && <p className="text-[10px] text-rose-600 mt-1">{fieldErrors.designation}</p>}
            </div>
          </div>
        </div>
      </div>
